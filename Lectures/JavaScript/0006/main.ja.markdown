...
* [5.2.JavaScriptの制御構文](#javascriptの制御文)
* [5.3.高階関数](#高階関数)
* [5.A.付録](#a.付録)
...
ハッシュ型については少しややこしいので後の方で整理して解説します。

### 5.1.4.JavaScriptの型変換
...
### 5.1.2.JavaScriptの型の分類: 値型と参照型とnull値
...
JavaScriptのハッシュ型で重要な事は、キーは常に文字列型、ヴァリューは任意の型
と言う点です。キーは文字列型なので、
...
こんな様なものもあります。これはハッシュ型として解釈される配列リテラルです。
つまりその実体は次の様なハッシュ型と等価です意味しています。

    > { "0":"abc", "1":"def", "2":"ghi", "3":345, "4":234, "5":123 }

<div class="note">
なお、厳密に言えば、言語仕様上は配列リテラルは「配列型」として定義されてはいるのですが、
実際にはハッシュ型の糖衣構文でしか無いので本節では敢えてその意図を伝える為に、
「配列リテラル」と言って紹介しました。
</div>
...
（ToDo: 質疑応答や後半に遭遇した面白い何かなどを可能ならば掲載する）
